...
// 覆盖：① 20 自定义命令方法由 COMMANDS 生成并透传；② plugin 命令（dialog/updater/webview）透传；
...
test('COMMANDS 集合 == lib.rs generate_handler! 注册集合（C17，前后端须同步）', () => {
  ...
  assert.equal(expected.length, 20, '应有 20 个自定义命令（僵尸 render_markdown 不在 handler 内）');
});
...
test('20 个自定义命令方法由 COMMANDS 生成并原样透传（C5/N29）', async () => {
  const calls = [];
  global.window = {
