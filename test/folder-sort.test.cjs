...
test('folder-sort: renderFolderTree 按 name 升序渲染，并显示大小/时间', async () => {
  ...
  try {
    ...
    w.__TAURI__.core.invoke = async (cmd, args) => {
      if (cmd === 'list_dir') return sampleEntries();
...
test('folder-ctx: 文件树节点右键显示 #context-menu-file-tree 并记录上下文路径', async () => {
  ...
  try {
    ...
    ed.workspaceFolder = '/root';
    await ed.renderFolderTree();
    // 检查 dev2 的 context-menu-file-tree 菜单
    ...
    // 使用 bubbles: true 让事件冒泡到容器，触发 dev2 的委托监听器
    const evt = new w.window.Event('contextmenu', { bubbles: true });
    evt.clientX = 10; evt.clientY = 10;
    fileNode.dispatchEvent(evt);

    // 检查 dev2 的文件树菜单是否显示
    assert.ok(!menu.classList.contains('hidden'), '右键后文件树菜单应显示');
    // 验证上下文状态被正确设置
    assert.strictEqual(ed._folderCtxPath, '/root/a.md', '应记录被右键文件的路径');
    assert.strictEqual(ed._folderCtxIsDir, false);
    assert.strictEqual(ed._fileTreeCtx?.path, '/root/a.md', 'dev2 的 _fileTreeCtx 也应记录路径');
  } finally { cleanup(w); }
});
